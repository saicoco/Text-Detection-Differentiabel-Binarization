...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, scale=1, train_mode=True):
        self.inplanes = 64
        ...
        self.conv_probability_head = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, padding=0)
        self.conv_threshold_head = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        probability_map = self.conv_probability_head(out)
        threshold_map = torch.sigmoid(self.conv_threshold_head(out))
        binarization_map = 50 * (torch.sigmoid(probability_map) - threshold_map)

        if not self.train_mode:
            probability_map = torch.sigmoid(probability_map)
            binarization_map = torch.sigmoid(binarization_map)

        return probability_map, threshold_map, binarization_map

...
def train(train_loader, model, criterion, optimizer, epoch, summary_writer):
    ...
    for batch_idx, (imgs, gt_texts, gt_kernels, training_masks, ori_imgs, mask_kernels) in enumerate(train_loader):
        ...
        # loss for binary_map
        binary_selected_masks = ohem_batch(binarization_map, gt_texts, training_masks)
        binary_selected_masks = Variable(binary_selected_masks.cuda())
        loss_binary = criterion(binarization_map, gt_texts, binary_selected_masks)
        
        # loss for threshold_map
        loss_thresh = L1_loss(threshold_map * mask_kernels, gt_kernels * mask_kernels)
        ...
        score_prob = cal_text_score(probability_map, gt_texts, training_masks, running_metric_text)
        score_binary = cal_text_score(binarization_map, gt_texts, training_masks, running_metric_text)
        ...
        if batch_idx % 20 == 0:
            ...
            summary_writer.add_images('gt/img', ori_imgs[:2], global_step=global_step)
            summary_writer.add_images('gt/score_map', torch.unsqueeze(gt_texts[:2], 1), global_step=global_step)
            summary_writer.add_images('gt/thresh_map', torch.unsqueeze(gt_kernels[:2], 1), global_step=global_step)
            summary_writer.add_images('predicition/score_map', torch.sigmoid(probability_map[:2]), global_step=global_step)
            summary_writer.add_images('predicition/binary_map', torch.sigmoid(binarization_map[:2]), global_step=global_step)
            summary_writer.add_images('predicition/threshold_map', torch.sigmoid(threshold_map[:2]), global_step=global_step)
            
            summary_writer.add_scalar('loss/text_loss', loss_probability, global_step=global_step)
            summary_writer.add_scalar('loss/binary_loss', loss_binary, global_step=global_step)
            summary_writer.add_scalar('loss/thresh_loss', loss_thresh, global_step=global_step)

            summary_writer.add_scalar('metric/acc_t', score_prob['Mean Acc'], global_step=global_step)
            summary_writer.add_scalar('metric/acc_b', score_binary['Mean Acc'], global_step=global_step)
            summary_writer.add_scalar('metric/iou_t', score_prob['Mean IoU'], global_step=global_step)
            summary_writer.add_scalar('metric/iou_b', score_binary['Mean IoU'], global_step=global_step)
    ...
    return (losses.avg, score_prob['Mean Acc'], score_binary['Mean Acc'], score_prob['Mean IoU'], score_binary['Mean IoU'])
...
def main(args):
    ...
    for epoch in range(start_epoch, args.n_epoch):
        ...
        train_loss, train_te_acc, train_ke_acc, train_te_iou, train_ke_iou = train(train_loader, model, dice_loss, optimizer, epoch, writer)
        save_checkpoint({
                'epoch': epoch + 1,
